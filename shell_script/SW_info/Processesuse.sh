...
indexName="processesuse"
tempPath="/root/you/Processesuse/"
...
cat ${tempPath}Jobs_Get_JsonData |grep '"ReleaseName": *\|\ },'|sed 's/\ },//g'|tr -d ' '|sed '/^$/d'|awk -F ':' '{print $2}'|sed 's/"//g'|sed 's/,//g' > ${tempPath}Jobs_Get_filterid
...
#陣列總數
finalProcessesNum=`echo ${#listSRobottotal[@]}`

echo ${finalProcessesNum}

PUT_BODY="{
  \"@timestamp\":\"${Nowtime}\",
  \"TotalProcessesNumber\":\"${finalProcessesNum}\"
}"
