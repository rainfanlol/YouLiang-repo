...
GetRawdata=`curl -k -X GET "https://oct1/odata/Jobs?%24filter=State%20ne%20'Successful'%20and%20State%20ne%20'Faulted'%20and%20State%20ne%20'Stopped'&%24select=HostMachineName%2CReleaseName%2CState" -H "accept: application/json" -H "Authorization: Bearer ${GetToken}"`

echo "${GetRawdata}"|python -m json.tool > ${tempPath}Jobs_Get_JsonData

cat ${tempPath}Jobs_Get_JsonData |grep '"State": "\|"ReleaseName": "\|"HostMachineName": "\|\ },'|sed 's/\ },/---------/g'|tr -d ' '|sed 's/ReleaseName/processesname/g' > ${tempPath}Jobs_Get_filterData
...
do
  ...
  filterData_head=`cat ${tempPath}Jobs_Get_filterData|grep -n State|head -n 1|awk -F ':' '{print $1}'`
